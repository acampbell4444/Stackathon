...
let aWt, bWt, cWt
export function MultiFlyAdv ({personA, personB, personC}) { 
    ...
    return (
      <div >
         ...
         <div className='container-fluid'>
            <h2 className='header'>The MultiFlyer Advantage Triple Bar</h2>
            <div>
            <form>
               <div>
                  <div className='row center'>
                    ...
                    <div className='well'>
                      ...
                      <Field name="personC" component='input' type="range" min='0' max='500' id='slider'/>
                    </div>
                    <div className='error' >
                	{
                		error&&(<p>{error}</p>)
            ...
            <div className='container-fluid barContainer'>
              ...
              {
	              show&&(
		              <p className={'spPosArrow indicators tripAdvPosArrowTop' } id={'tripAdvArrowLeft' + position}>
...
function tripBalance(a,b,c) {
 ...
 function justDubs(a,b){
 	...
 	if(diff>230){
 		position = 9
 		spotOne = spotTwo = spotThree = spotFour = spotFive = spotSix = error = null
 		error = 'Cannot Have a difference of More than 230 lbs with doubles using this bar.  Your Current Difference is ' + diff + ' lbs'
 	}
 	switch(true) {
      case (diff<=10):
...
function trips (a,b,c){
  ...
  if(diff>110){
  		spotOne = spotTwo = spotThree = spotFour = spotFive = spotSix = error = null
  		position = 9
 		error = 'This weight differences are too extreme for this triple combination when using the MultiFlyer Regular Bar. At least two passengers must be within 110 lbs of each other'
 	}
  switch(true) {
      case (diff<=10):
      	position=0
